...
export function CodeEditor({ code, onChange, htmlCode, onHtmlChange, onRun, onReset, onShowHint, isRunning }) {
  ...
  return (
    <div className="flex flex-col h-full bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-2xl">
      ...
      <div className="bg-slate-950 px-4 py-3 border-b border-slate-800 flex items-center justify-between gap-3 shrink-0">
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <button
            onClick={onRun}
            disabled={isRunning}
            className="flex items-center gap-2 bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 text-slate-950 font-bold text-xs px-4 py-1.5 rounded-lg shadow-lg shadow-amber-500/20 transition-all font-mono active:scale-95 disabled:opacity-50 shrink-0"
      ...
      {/* Editor Body */}
      <div className="flex-1 min-h-[300px] relative bg-[#1e1e1e]" onKeyDown={handleKeyDown}>
        <Editor
          key={activeTab}
          height="100%"
